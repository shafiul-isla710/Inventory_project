...
class ResetPasswordController extends Controller
{
    ...
    public function sentOtp(SentOtpRequest $request){
        try{
            ...
            return $this->responseWithSuccess(true,'OTP sent successfully');
        }
        catch(\Exception $e){
    ...
    public function verifyOtp(VerifyOtpRequest $request){
        try{
            ...
            return $this->responseWithSuccess(true,'OTP verified successfully', 200)->cookie('reset_password_token', $token['token'], $exp);
        }
        catch(\Exception $e){
            Log::error($e->getMessage().''.$e->getFile().':'.$e->getLine());
            return $this->responseWithError(false,'Something went wrong. Please try again.', [], 500);
    ...
    public function resetPassword(ResetPasswordRequest $request)
    {
        try{
            ...
            if(!$cookie){
                return $this->responseWithError(false,'Unauthorized', [], 401);
            }
            ...
            return $this->responseWithSuccess(true,'Password reset successfully', 200)->withoutCookie('reset_password_token');
        }
        catch(\Exception $e){
            Log::error($e->getMessage().''.$e->getFile().':'.$e->getLine());
            return $this->responseWithError(false,'Something went wrong. Please try again.', [], 500);
